...
VOID ExecuteConsole() {
	...
	for (int i = 0; i < argc; i++) {
		...
		// update DLL payload resource
			...
			p->lpPayload = std::wstring(argv[i + 1]);
	...
	if ((p->lpProcessName.empty() && p->lpPayload.empty()) ||
		!p->lpProcessName.empty() && !p->lpPayload.empty())
		PrintUsage(argv[0]);
	...
	} else if (!p->lpPayload.empty()) {
		Debug(L"You've selected to update your payload with %s\n", p->lpPayload.c_str());
		Injector *i = new Injector(p->lpPayload, p->dwOptions);
		UpdatePayload(i, p->lpPayload);
		delete i;
	}
